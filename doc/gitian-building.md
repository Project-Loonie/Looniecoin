...
Clone the git repositories for loonie and gitian and then checkout the loonie version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the loonie repository
...
To build LNI (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the loonie repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/loonie/.git/
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/loonie-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/loonie-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/loonie-build.assert
```
...
[loonie/gitian.sigs](https://github.com/loonie-crypto/gitian.sigs/) repository, or if that's not possible to create a pull
request.
