...
// The main object for accessing mixTX
CmixTXPool miXtxPool;
// A helper object for signing messages from Masternodes
CmiXtxSigner miXtxSigner;
// The current mixTXs in progress on the network
std::vector<CmixTXQueue> vecmixTXQueue;
// Keep track of the used Masternodes
std::vector<CTxIn> vecMasternodesUsed;
// Keep track of the scanning errors I've seen
map<uint256, CmixTXBroadcastTx> mapmixTXBroadcastTxes;
// Keep track of the active Masternode
CActiveMasternode activeMasternode;

/* *** BEGIN MIXTX MAGIC - LNI **********
    Copyright (c) 2014-2015, Dash Developers
        eduffield - evan@dashpay.io
        udjinm6   - udjinm6@dashpay.io
*/

void CmixTXPool::ProcessMessagemixTX(CNode* pfrom, std::string& strCommand, CDataStream& vRecv)
{
    if (fLiteMode) return; //disable all mixTX/Masternode related functionality
    if (!masternodeSync.IsBlockchainSynced()) return;

    if (strCommand == "dsa") { //mixTX Accept Into Pool
    ...
    } else if (strCommand == "dsq") { //mixTX Queue
        TRY_LOCK(cs_mixTX, lockRecv);
        if (!lockRecv) return;
        ...
        if (dsq.ready) {
            ...
            if (state == POOL_STATUS_QUEUE) {
                LogPrint("mixTX", "mixTX queue is ready - %s\n", addr.ToString());
                PreparemixTXDenominate();
            }
        } else {
            BOOST_FOREACH (CmixTXQueue q, vecmixTXQueue) {
                if (q.vin == dsq.vin) return;
            }

            LogPrint("mixTX", "dsq last %d last2 %d count %d\n", pmn->nLastDsq, pmn->nLastDsq + mnodeman.size() / 5, mnodeman.nDsqCount);
            //don't allow a few nodes to dominate the queuing process
            if (pmn->nLastDsq != 0 &&
                pmn->nLastDsq + mnodeman.CountEnabled(ActiveProtocol()) / 5 > mnodeman.nDsqCount) {
                LogPrint("mixTX", "dsq -- Masternode sending too many dsq messages. %s \n", pmn->addr.ToString());
                return;
            }
            mnodeman.nDsqCount++;
            ...
            LogPrint("mixTX", "dsq - new mixTX queue object - %s\n", addr.ToString());
            vecmixTXQueue.push_back(dsq);
            dsq.Relay();
            dsq.time = GetTime();
        }

    } else if (strCommand == "dsi") { //miXtx vIn
        int errorID;
        ...
        //check it like a transaction
        {
            ...
            if (nValueIn > MIXTX_POOL_MAX) {
                LogPrintf("dsi -- more than mixTX pool max! %s\n", tx.ToString());
                errorID = ERR_MAXIMUM;
                pfrom->PushMessage("dssu", sessionID, GetState(), GetEntriesCount(), MASTERNODE_REJECTED, errorID);
                return;
            }
    ...
    } else if (strCommand == "dssu") { //mixTX status update
        if (pfrom->nVersion < ActiveProtocol()) {
            return;
        }
        ...
        BOOST_FOREACH (const CTxIn item, sigs) {
            ...
            LogPrint("mixTX", " -- sigs count %d %d\n", (int)sigs.size(), count);
            count++;
        }

        if (success) {
            miXtxPool.Check();
            RelayStatus(miXtxPool.sessionID, miXtxPool.GetState(), miXtxPool.GetEntriesCount(), MASTERNODE_RESET);
        }
    } else if (strCommand == "dsf") { //mixTX Final tx
        if (pfrom->nVersion < ActiveProtocol()) {
            return;
        }
        ...
        if (sessionID != sessionIDMessage) {
            LogPrint("mixTX", "dsf - message doesn't match current mixTX session %d %d\n", sessionID, sessionIDMessage);
            return;
        }
    ...
    } else if (strCommand == "dsc") { //mixTX Complete
        ...
        if (sessionID != sessionIDMessage) {
            LogPrint("mixTX", "dsc - message doesn't match current mixTX session %d %d\n", miXtxPool.sessionID, sessionIDMessage);
            return;
        }

        miXtxPool.CompletedTransaction(error, errorID);
    }
}

int randomizeList(int i) { return std::rand() % i; }

void CmixTXPool::Reset()
{
    cachedLastSuccess = 0;
    lastNewBlock = 0;
...
bool CmixTXPool::SetCollateralAddress(std::string strAddress)
{
    CBitcoinAddress address;
    if (!address.SetString(strAddress)) {
        LogPrintf("CmixTXPool::SetCollateralAddress - Invalid mixTX collateral address\n");
        return false;
    }
...
//
// Unlock coins after mixTX fails or succeeds
//
void CmixTXPool::UnlockCoins()
{
    while (true) {
...
std::string CmixTXPool::GetStatus()
{
    static int showingmiXtxMessage = 0;
    showingmiXtxMessage += 10;
    std::string suffix = "";
    ...
    case POOL_STATUS_IDLE:
        return _("mixTX is idle.");
    case POOL_STATUS_ACCEPTING_ENTRIES:
        if (entriesCount == 0) {
            showingmiXtxMessage = 0;
            return strAutoDenomResult;
        } else if (lastEntryAccepted == 1) {
            if (showingmiXtxMessage % 10 > 8) {
                lastEntryAccepted = 0;
                showingmiXtxMessage = 0;
            }
            return _("mixTX request complete:") + " " + _("Your transaction was accepted into the pool!");
        } else {
            std::string suffix = "";
            if (showingmiXtxMessage % 70 <= 40)
                return strprintf(_("Submitted following entries to masternode: %u / %d"), entriesCount, GetMaxPoolTransactions());
            else if (showingmiXtxMessage % 70 <= 50)
                suffix = ".";
            else if (showingmiXtxMessage % 70 <= 60)
                suffix = "..";
            else if (showingmiXtxMessage % 70 <= 70)
                suffix = "...";
    ...
    case POOL_STATUS_ERROR:
        return _("mixTX request incomplete:") + " " + lastMessage + " " + _("Will retry...");
    case POOL_STATUS_SUCCESS:
        return _("mixTX request complete:") + " " + lastMessage;
    case POOL_STATUS_QUEUE:
        if (showingmiXtxMessage % 70 <= 30)
            suffix = ".";
        else if (showingmiXtxMessage % 70 <= 50)
            suffix = "..";
        else if (showingmiXtxMessage % 70 <= 70)
...
//
// Check the mixTX progress and send client updates if a Masternode
//
void CmixTXPool::Check()
{
    if (fMasterNode) LogPrint("mixTX", "CmixTXPool::Check() - entries count %lu\n", entries.size());
    //printf("CmixTXPool::Check() %d - %d - %d\n", state, anonTx.CountEntries(), GetTimeMillis()-lastTimeChanged);

    if (fMasterNode) {
        LogPrint("mixTX", "CmixTXPool::Check() - entries count %lu\n", entries.size());

        // If entries is full, then move on to the next phase
        if (state == POOL_STATUS_ACCEPTING_ENTRIES && (int)entries.size() >= GetMaxPoolTransactions()) {
            LogPrint("mixTX", "CmixTXPool::Check() -- TRYING TRANSACTION \n");
    ...
    if (state == POOL_STATUS_FINALIZE_TRANSACTION) {
        LogPrint("mixTX", "CmixTXPool::Check() -- FINALIZE TRANSACTIONS\n");
        ...
        if (fMasterNode) {
            ...
            LogPrint("mixTX", "Transaction 1: %s\n", txNew.ToString());
    ...
    if (fMasterNode && state == POOL_STATUS_SIGNING && SignaturesComplete()) {
        LogPrint("mixTX", "CmixTXPool::Check() -- SIGNING\n");
    ...
    if ((state == POOL_STATUS_ERROR || state == POOL_STATUS_SUCCESS) && GetTimeMillis() - lastTimeChanged >= 10000) {
        LogPrint("mixTX", "CmixTXPool::Check() -- timeout, RESETTING\n");
        UnlockCoins();
        SetNull();
...
void CmixTXPool::CheckFinalTransaction()
{
    ...
    {
        LogPrint("mixTX", "Transaction 2: %s\n", txNew.ToString());

        // See if the transaction is valid
        if (!txNew.AcceptToMemoryPool(false, true, true)) {
            LogPrintf("CmixTXPool::Check() - CommitTransaction : Error: Transaction not valid\n");
            SetNull();
        ...
        LogPrintf("CmixTXPool::Check() -- IS MASTER -- TRANSMITTING MIXTX\n");
        ...
        if (!miXtxSigner.SetKey(strMasterNodePrivKey, strError, key2, pubkey2)) {
            LogPrintf("CmixTXPool::Check() - ERROR: Invalid Masternodeprivkey: '%s'\n", strError);
            return;
        }

        if (!miXtxSigner.SignMessage(strMessage, strError, vchSig, key2)) {
            LogPrintf("CmixTXPool::Check() - Sign message failed\n");
            return;
        }

        if (!miXtxSigner.VerifyMessage(pubkey2, vchSig, strMessage, strError)) {
            LogPrintf("CmixTXPool::Check() - Verify message failed\n");
            return;
        }

        if (!mapmixTXBroadcastTxes.count(txNew.GetHash())) {
            CmixTXBroadcastTx dstx;
            dstx.tx = txNew;
            dstx.vin = activeMasternode.vin;
            dstx.vchSig = vchSig;
            dstx.sigTime = sigTime;

            mapmixTXBroadcastTxes.insert(make_pair(txNew.GetHash(), dstx));
        }
        ...
        // Reset
        LogPrint("mixTX", "CmixTXPool::Check() -- COMPLETED -- RESETTING\n");
        SetNull();
        RelayStatus(sessionID, GetState(), GetEntriesCount(), MASTERNODE_RESET);
    }
}
...
//
// Why bother? mixTX uses collateral to ensure abuse to the process is kept to a minimum.
// The submission and signing stages in mixTX are completely separate. In the cases where
...
void CmixTXPool::ChargeFees()
{
    ...
    if (state == POOL_STATUS_ACCEPTING_ENTRIES) {
        BOOST_FOREACH (const CTransaction& txCollateral, vecSessionCollateral) {
            bool found = false;
            BOOST_FOREACH (const CmiXtxEntry& v, entries) {
            ...
            if (!found) {
                LogPrintf("CmixTXPool::ChargeFees -- found uncooperative node (didn't send transaction). Found offence.\n");
                offences++;
            }
        }
    }
    ...
    if (state == POOL_STATUS_SIGNING) {
        ...
        BOOST_FOREACH (const CmiXtxEntry v, entries) {
            BOOST_FOREACH (const CTxDSIn s, v.sev) {
                if (!s.fHasSig) {
                    LogPrintf("CmixTXPool::ChargeFees -- found uncooperative node (didn't sign). Found offence\n");
                    offences++;
                }
            }
        }
    ...
    if (state == POOL_STATUS_ACCEPTING_ENTRIES) {
        BOOST_FOREACH (const CTransaction& txCollateral, vecSessionCollateral) {
            ...
            // This queue entry didn't send us the promised transaction
            if (!found && r > target) {
                LogPrintf("CmixTXPool::ChargeFees -- found uncooperative node (didn't send transaction). charging fees.\n");
                ...
                if (!wtxCollateral.AcceptToMemoryPool(true)) {
                    ...
                    LogPrintf("CmixTXPool::ChargeFees() : Error: Transaction not valid");
                }
        ...
        BOOST_FOREACH (const CmiXtxEntry v, entries) {
            BOOST_FOREACH (const CTxDSIn s, v.sev) {
                if (!s.fHasSig && r > target) {
                    LogPrintf("CmixTXPool::ChargeFees -- found uncooperative node (didn't sign). charging fees.\n");
...
//  - mixTX is completely free, to pay miners we randomly pay the collateral of users.
void CmixTXPool::ChargeRandomFees()
{
    if (fMasterNode) {
        int i = 0;
        ...
        BOOST_FOREACH (const CTransaction& txCollateral, vecSessionCollateral) {
            ...
            /*
                ...
                Being that mixTX has "no fees" we need to have some kind of cost associated
                with using it to stop abuse. Otherwise it could serve as an attack vector and
                allow endless transaction that would bloat LNI and make it unusable. To
                stop these kinds of attacks 1 in 10 successful transactions are charged. This
                adds up to a cost of 0.001 LNI per transaction on average.
            */
            if (r <= 10) {
                LogPrintf("CmixTXPool::ChargeRandomFees -- charging random fees. %u\n", i);
                ...
                if (!wtxCollateral.AcceptToMemoryPool(true)) {
                    // This must not fail. The transaction has already been signed and recorded.
                    LogPrintf("CmixTXPool::ChargeRandomFees() : Error: Transaction not valid");
                }
                wtxCollateral.RelayWalletTransaction();
            }
        }
...
//
// Check for various timeouts (queue objects, mixTX, etc)
//
void CmixTXPool::CheckTimeout()
{
    if (!fEnablemixTX && !fMasterNode) return;

    // catching hanging sessions
    if (!fMasterNode) {
        switch (state) {
        case POOL_STATUS_TRANSMISSION:
            LogPrint("mixTX", "CmixTXPool::CheckTimeout() -- Session complete -- Running Check()\n");
            Check();
            break;
        case POOL_STATUS_ERROR:
            LogPrint("mixTX", "CmixTXPool::CheckTimeout() -- Pool error -- Running Check()\n");
            Check();
            break;
        case POOL_STATUS_SUCCESS:
            LogPrint("mixTX", "CmixTXPool::CheckTimeout() -- Pool success -- Running Check()\n");
            Check();
            break;
        }
    }

    // check mixTX queue objects for timeouts
    int c = 0;
    vector<CmixTXQueue>::iterator it = vecmixTXQueue.begin();
    while (it != vecmixTXQueue.end()) {
        if ((*it).IsExpired()) {
            LogPrint("mixTX", "CmixTXPool::CheckTimeout() : Removing expired queue entry - %d\n", c);
            it = vecmixTXQueue.erase(it);
        } else
            ++it;
        c++;
    }
    ...
    if (state == POOL_STATUS_ACCEPTING_ENTRIES || state == POOL_STATUS_QUEUE) {
        ...
        vector<CmiXtxEntry>::iterator it2 = entries.begin();
        while (it2 != entries.end()) {
            if ((*it2).IsExpired()) {
                LogPrint("mixTX", "CmixTXPool::CheckTimeout() : Removing expired entry - %d\n", c);
        ...
        if (GetTimeMillis() - lastTimeChanged >= (MIXTX_QUEUE_TIMEOUT * 1000) + addLagTime) {
            UnlockCoins();
            SetNull();
        }
    } else if (GetTimeMillis() - lastTimeChanged >= (MIXTX_QUEUE_TIMEOUT * 1000) + addLagTime) {
        LogPrint("mixTX", "CmixTXPool::CheckTimeout() -- Session timed out (%ds) -- resetting\n", MIXTX_QUEUE_TIMEOUT);
        UnlockCoins();
        SetNull();
    ...
    if (state == POOL_STATUS_SIGNING && GetTimeMillis() - lastTimeChanged >= (MIXTX_SIGNING_TIMEOUT * 1000) + addLagTime) {
        LogPrint("mixTX", "CmixTXPool::CheckTimeout() -- Session timed out (%ds) -- restting\n", MIXTX_SIGNING_TIMEOUT);
        ChargeFees();
        UnlockCoins();
        SetNull();
...
//
void CmixTXPool::CheckForCompleteQueue()
{
    if (!fEnablemixTX && !fMasterNode) return;
...
bool CmixTXPool::SignatureValid(const CScript& newSig, const CTxIn& newVin)
{
    CMutableTransaction txNew;
    txNew.vin.clear();
    txNew.vout.clear();
    ...
    BOOST_FOREACH (CmiXtxEntry& e, entries) {
    ...
    if (found >= 0) { //might have to do this one input at a time?
        ...
        LogPrint("mixTX", "CmixTXPool::SignatureValid() - Sign with sig %s\n", newSig.ToString().substr(0, 24));
        if (!VerifyScript(txNew.vin[n].scriptSig, sigPubKey, SCRIPT_VERIFY_P2SH | SCRIPT_VERIFY_STRICTENC, MutableTransactionSignatureChecker(&txNew, n))) {
            LogPrint("mixTX", "CmixTXPool::SignatureValid() - Signing - Error signing input %u\n", n);
            return false;
        }
    }

    LogPrint("mixTX", "CmixTXPool::SignatureValid() - Signing - Successfully validated input\n");
    return true;
}

// check to make sure the collateral provided by the client is valid
bool CmixTXPool::IsCollateralValid(const CTransaction& txCollateral)
{
    ...
    BOOST_FOREACH (const CTxOut o, txCollateral.vout) {
        ...
        if (!o.scriptPubKey.IsNormalPaymentScript()) {
            LogPrintf("CmixTXPool::IsCollateralValid - Invalid Script %s\n", txCollateral.ToString());
            return false;
        }
    }
    ...
    if (missingTx) {
        LogPrint("mixTX", "CmixTXPool::IsCollateralValid - Unknown inputs in collateral transaction - %s\n", txCollateral.ToString());
        return false;
    }

    //collateral transactions are required to pay out MIXTX_COLLATERAL as a fee to the miners
    if (nValueIn - nValueOut < MIXTX_COLLATERAL) {
        LogPrint("mixTX", "CmixTXPool::IsCollateralValid - did not include enough fees in transaction %d\n%s\n", nValueOut - nValueIn, txCollateral.ToString());
        return false;
    }

    LogPrint("mixTX", "CmixTXPool::IsCollateralValid %s\n", txCollateral.ToString());

    {
        LOCK(cs_main);
        CValidationState state;
        if (!AcceptableInputs(mempool, state, txCollateral, true, NULL)) {
            if (fDebug) LogPrintf("CmixTXPool::IsCollateralValid - didn't pass IsAcceptable\n");
            return false;
        }
    }
...
//
bool CmixTXPool::AddEntry(const std::vector<CTxIn>& newInput, const int64_t& nAmount, const CTransaction& txCollateral, const std::vector<CTxOut>& newOutput, int& errorID)
{
    if (!fMasterNode) return false;

    BOOST_FOREACH (CTxIn in, newInput) {
        if (in.prevout.IsNull() || nAmount < 0) {
            LogPrint("mixTX", "CmixTXPool::AddEntry - input not valid!\n");
    ...
    if (!IsCollateralValid(txCollateral)) {
        LogPrint("mixTX", "CmixTXPool::AddEntry - collateral not valid!\n");
    ...
    if ((int)entries.size() >= GetMaxPoolTransactions()) {
        LogPrint("mixTX", "CmixTXPool::AddEntry - entries is full!\n");
    ...
    BOOST_FOREACH (CTxIn in, newInput) {
        LogPrint("mixTX", "looking for vin -- %s\n", in.ToString());
        BOOST_FOREACH (const CmiXtxEntry& v, entries) {
            BOOST_FOREACH (const CTxDSIn& s, v.sev) {
                if ((CTxIn)s == in) {
                    LogPrint("mixTX", "CmixTXPool::AddEntry - found in vin\n");
    ...
    LogPrint("mixTX", "CmixTXPool::AddEntry -- adding %s\n", newInput[0].ToString());
    errorID = MSG_ENTRIES_ADDED;

    return true;
}

bool CmixTXPool::AddScriptSig(const CTxIn& newVin)
{
    LogPrint("mixTX", "CmixTXPool::AddScriptSig -- new sig  %s\n", newVin.scriptSig.ToString().substr(0, 24));


    BOOST_FOREACH (const CmiXtxEntry& v, entries) {
        BOOST_FOREACH (const CTxDSIn& s, v.sev) {
            if (s.scriptSig == newVin.scriptSig) {
                LogPrint("mixTX", "CmixTXPool::AddScriptSig - already exists\n");
                return false;
            }
        }
    }
    ...
    if (!SignatureValid(newVin.scriptSig, newVin)) {
        LogPrint("mixTX", "CmixTXPool::AddScriptSig - Invalid Sig\n");
        return false;
    }

    LogPrint("mixTX", "CmixTXPool::AddScriptSig -- sig %s\n", newVin.ToString());

    BOOST_FOREACH (CTxIn& vin, finalTransaction.vin) {
        if (newVin.prevout == vin.prevout && vin.nSequence == newVin.nSequence) {
            vin.scriptSig = newVin.scriptSig;
            vin.prevPubKey = newVin.prevPubKey;
            LogPrint("mixTX", "CmiXtxPool::AddScriptSig -- adding to finalTransaction  %s\n", newVin.scriptSig.ToString().substr(0, 24));
        }
    }
    for (unsigned int i = 0; i < entries.size(); i++) {
        if (entries[i].AddSig(newVin)) {
            LogPrint("mixTX", "CmiXtxPool::AddScriptSig -- adding  %s\n", newVin.scriptSig.ToString().substr(0, 24));
            return true;
        }
    }

    LogPrintf("CmixTXPool::AddScriptSig -- Couldn't set sig!\n");
    return false;
}

// Check to make sure everything is signed
bool CmixTXPool::SignaturesComplete()
{
    BOOST_FOREACH (const CmiXtxEntry& v, entries) {
        BOOST_FOREACH (const CTxDSIn& s, v.sev) {
...
//
// Execute a mixTX denomination via a Masternode.
// This is only ran from clients
//
void CmixTXPool::SendmixTXDenominate(std::vector<CTxIn>& vin, std::vector<CTxOut>& vout, int64_t amount)
{
    if (fMasterNode) {
        LogPrintf("CmixTXPool::SendmixTXDenominate() - mixTX from a Masternode is not supported currently.\n");
        return;
    }

    if (txCollateral == CMutableTransaction()) {
        LogPrintf("CmixTXPool:SendmixTXDenominate() - mixTX collateral not set");
        return;
    }
    ...
    if (!sessionFoundMasternode) {
        LogPrintf("CmixTXPool::SendmixTXDenominate() - No Masternode has been selected yet.\n");
        UnlockCoins();
        SetNull();
        return;
    }
    ...
    if (!CheckDiskSpace()) {
        ...
        SetNull();
        fEnablemixTX = false;
        LogPrintf("CmixTXPool::SendmixTXDenominate() - Not enough disk space, disabling mixTX.\n");
        return;
    }

    UpdateState(POOL_STATUS_ACCEPTING_ENTRIES);

    LogPrintf("CmixTXPool::SendmixTXDenominate() - Added transaction to pool.\n");
...
// Incoming message from Masternode updating the progress of mixTX
//    newAccepted:  -1 mean's it'n not a "transaction accepted/not accepted" message, just a standard update
//                  0 means transaction was not accepted
//                  1 means transaction was accepted

bool CmixTXPool::StatusUpdate(int newState, int newEntriesCount, int newAccepted, int& errorID, int newSessionID)
{
    if (fMasterNode) return false;
    ...
    if (newAccepted != -1) {
        ...
        if (newAccepted == 1 && newSessionID != 0) {
            ...
            LogPrintf("CmixTXPool::StatusUpdate - set sessionID to %d\n", sessionID);
    ...
    if (newState == POOL_STATUS_ACCEPTING_ENTRIES) {
        if (newAccepted == 1) {
            LogPrintf("CmixTXPool::StatusUpdate - entry accepted! \n");
            sessionFoundMasternode = true;
            //wait for other users. Masternode will report when ready
            UpdateState(POOL_STATUS_QUEUE);
        } else if (newAccepted == 0 && sessionID == 0 && !sessionFoundMasternode) {
            LogPrintf("CmixTXPool::StatusUpdate - entry not accepted by Masternode \n");
            UnlockCoins();
            UpdateState(POOL_STATUS_ACCEPTING_ENTRIES);
...
//
bool CmixTXPool::SignFinalTransaction(CTransaction& finalTransactionNew, CNode* node)
{
    if (fMasterNode) return false;

    finalTransaction = finalTransactionNew;
    LogPrintf("CmixTXPool::SignFinalTransaction %s", finalTransaction.ToString());

    vector<CTxIn> sigs;

    //make sure my inputs/outputs are present, otherwise refuse to sign
    BOOST_FOREACH (const CmiXtxEntry e, entries) {
        BOOST_FOREACH (const CTxDSIn s, e.sev) {
            ...
            if (mine >= 0) { //might have to do this one input at a time?
                ...
                if (foundOutputs < targetOuputs || nValue1 != nValue2) {
                    ...
                    LogPrintf("CmixTXPool::Sign - My entries are not correct! Refusing to sign. %d entries %d target. \n", foundOutputs, targetOuputs);
                    UnlockCoins();
                    SetNull();
...
void CmixTXPool::NewBlock()
{
    LogPrint("mixTX", "CmixTXPool::NewBlock \n");

    //we we're processing lots of blocks, we'll just leave
    if (GetTime() - lastNewBlock < 10) return;
    lastNewBlock = GetTime();

    miXtxPool.CheckTimeout();
}

// mixTX transaction was completed (failed or successful)
void CmixTXPool::CompletedTransaction(bool error, int errorID)
{
    if (fMasterNode) return;
...
void CmixTXPool::ClearLastMessage()
{
    lastMessage = "";
}

//
// Passively run mixTX in the background to anonymize funds based on the given configuration.
//
// This does NOT run by default for daemons, only for QT.
//
bool CmixTXPool::DoAutomaticDenominating(bool fDryRun)
{
    if (!fEnablemixTX) return false;
    if (fMasterNode) return false;
    if (state == POOL_STATUS_ERROR || state == POOL_STATUS_SUCCESS) return false;
    ...
    if (chainActive.Tip()->nHeight - cachedLastSuccess < minBlockSpacing) {
        LogPrintf("CmixTXPool::DoAutomaticDenominating - Last successful mixTX action was too recent\n");
        strAutoDenomResult = _("Last successful mixTX action was too recent.");
        return false;
    }

    if (mnodeman.size() == 0) {
        LogPrint("mixTX", "CmixTXPool::DoAutomaticDenominating - No Masternodes detected\n");
    ...
    // should not be less than fees in MIXTX_COLLATERAL + few (lets say 5) smallest denoms
    CAmount nLowestDenom = MIXTX_COLLATERAL + miXtxDenominations[miXtxDenominations.size() - 1] * 5;

    // if there are no OBF collateral inputs yet
    if (!pwalletMain->HasCollateralInputs())
        // should have some additional amount for them
        nLowestDenom += MIXTX_COLLATERAL * 4;

    CAmount nBalanceNeedsAnonymized = nAnonymizeLoonieAmount * COIN - pwalletMain->GetAnonymizedBalance();

    // if balanceNeedsAnonymized is more than pool max, take the pool max
    if (nBalanceNeedsAnonymized > MIXTX_POOL_MAX) nBalanceNeedsAnonymized = MIXTX_POOL_MAX;
    ...
    LogPrint("mixTX", "DoAutomaticDenominating : nLowestDenom=%d, nBalanceNeedsAnonymized=%d\n", nLowestDenom, nBalanceNeedsAnonymized);

    // select coins that should be given to the pool
    if (!pwalletMain->SelectCoinsDark(nValueMin, nBalanceNeedsAnonymized, vCoins, nValueIn, 0, nmixTXRounds)) {
        nValueIn = 0;
        vCoins.clear();
    ...
    if (!sessionFoundMasternode) {
        ...
        if (nUseQueue > 33) {
            ...
            BOOST_FOREACH (CmixTXQueue& dsq, vecmixTXQueue) {
                ...
                if (!pwalletMain->SelectCoinsByDenominations(dsq.nDenom, nValueMin, nBalanceNeedsAnonymized, vTempCoins, vTempCoins2, nValueIn, 0, nmixTXRounds)) {
...
bool CmixTXPool::PreparemixTXDenominate()
{
    std::string strError = "";
    // Submit transaction to the pool if we get here
    // Try to use only inputs with the same number of rounds starting from lowest number of rounds possible
    for (int i = 0; i < nmixTXRounds; i++) {
        strError = pwalletMain->PreparemixTXDenominate(i, i + 1);
        LogPrintf("DoAutomaticDenominating : Running mixTX denominate for %d rounds. Return '%s'\n", i, strError);
        if (strError == "") return true;
    }

    // We failed? That's strange but let's just make final attempt and try to mix everything
    strError = pwalletMain->PreparemixTXDenominate(0, nmixTXRounds);
    LogPrintf("DoAutomaticDenominating : Running mixTX denominate for all rounds. Return '%s'\n", strError);
    if (strError == "") return true;
...
bool CmixTXPool::SendRandomPaymentToSelf()
{
...
bool CmixTXPool::MakeCollateralAmounts()
{
    CWalletTx wtx;
    int64_t nFeeRet = 0;
    std::string strFail = "";
    ...
    vecSend.push_back(make_pair(scriptCollateral, MIXTX_COLLATERAL * 4));
...
bool CmixTXPool::CreateDenominated(int64_t nTotalValue)
{
    CWalletTx wtx;
    int64_t nFeeRet = 0;
    std::string strFail = "";
    ...
    if (!pwalletMain->HasCollateralInputs()) {
        vecSend.push_back(make_pair(scriptCollateral, MIXTX_COLLATERAL * 4));
        nValueLeft -= MIXTX_COLLATERAL * 4;
    }

    // ****** Add denoms ************ /
    BOOST_REVERSE_FOREACH (int64_t v, miXtxDenominations) {
        int nOutputs = 0;

        // add each output up to 10 times until it can't be added again
        while (nValueLeft - v >= MIXTX_COLLATERAL && nOutputs <= 10) {
            CScript scriptDenom;
            CPubKey vchPubKey;
...
bool CmixTXPool::IsCompatibleWithEntries(std::vector<CTxOut>& vout)
{
...
bool CmixTXPool::IsCompatibleWithSession(int64_t nDenom, CTransaction txCollateral, int& errorID)
{
    if (nDenom == 0) return false;

    LogPrintf("CmixTXPool::IsCompatibleWithSession - sessionDenom %d sessionUsers %d\n", sessionDenom, sessionUsers);

    if (!unitTest && !IsCollateralValid(txCollateral)) {
        LogPrint("mixTX", "CmixTXPool::IsCompatibleWithSession - collateral not valid!\n");
        errorID = ERR_INVALID_COLLATERAL;
        return false;
    }
    ...
    if ((state != POOL_STATUS_ACCEPTING_ENTRIES && state != POOL_STATUS_QUEUE) || sessionUsers >= GetMaxPoolTransactions()) {
        ...
        LogPrintf("CmixTXPool::IsCompatibleWithSession - incompatible mode, return false %d %d\n", state != POOL_STATUS_ACCEPTING_ENTRIES, sessionUsers >= GetMaxPoolTransactions());
        return false;
    }
    ...
    LogPrintf("CmiXtxPool::IsCompatibleWithSession - compatible\n");
...
void CmixTXPool::GetDenominationsToString(int nDenom, std::string& strDenom)
{
...
int CmixTXPool::GetDenominations(const std::vector<CTxDSOut>& vout)
{
...
int CmixTXPool::GetDenominations(const std::vector<CTxOut>& vout, bool fSingleRandomDenom)
{
    std::vector<pair<int64_t, int> > denomUsed;

    // make a list of denominations, with zero uses
    BOOST_FOREACH (int64_t d, miXtxDenominations)
...
int CmixTXPool::GetDenominationsByAmounts(std::vector<int64_t>& vecAmount)
{
    CScript e = CScript();
...
int CmixTXPool::GetDenominationsByAmount(int64_t nAmount, int nDenomTarget)
{
    CScript e = CScript();
...
std::string CmixTXPool::GetMessageByID(int messageID)
{
    switch (messageID) {
    ...
    case ERR_MAXIMUM:
        return _("Value more than mixTX pool maximum allows.");
    case ERR_MN_LIST:
    ...
    case ERR_RECENT:
        return _("Last mixTX was too recent.");
    case ERR_SESSION:
...
bool CmiXtxSigner::IsVinAssociatedWithPubkey(CTxIn& vin, CPubKey& pubkey)
{
    CScript payee2;
...
bool CmiXtxSigner::SetKey(std::string strSecret, std::string& errorMessage, CKey& key, CPubKey& pubkey)
{
...
bool CmiXtxSigner::GetKeysFromSecret(std::string strSecret, CKey& keyRet, CPubKey& pubkeyRet)
{
    CBitcoinSecret vchSecret;
...
bool CmiXtxSigner::SignMessage(std::string strMessage, std::string& errorMessage, vector<unsigned char>& vchSig, CKey key)
{
...
bool CmiXtxSigner::VerifyMessage(CPubKey pubkey, vector<unsigned char>& vchSig, std::string strMessage, std::string& errorMessage)
{
    CHashWriter ss(SER_GETHASH, 0);
    ss << strMessageMagic;
    ss << strMessage;
    ...
    if (fDebug && pubkey2.GetID() != pubkey.GetID())
        LogPrintf("CmiXtxSigner::VerifyMessage -- keys don't match: %s %s\n", pubkey2.GetID().ToString(), pubkey.GetID().ToString());

    return (pubkey2.GetID() == pubkey.GetID());
}

bool CmixTXQueue::Sign()
{
    if (!fMasterNode) return false;
    ...
    if (!miXtxSigner.SetKey(strMasterNodePrivKey, errorMessage, key2, pubkey2)) {
        LogPrintf("CmixTXQueue():Relay - ERROR: Invalid Masternodeprivkey: '%s'\n", errorMessage);
        return false;
    }

    if (!miXtxSigner.SignMessage(strMessage, errorMessage, vchSig, key2)) {
        LogPrintf("CmixTXQueue():Relay - Sign message failed");
        return false;
    }

    if (!miXtxSigner.VerifyMessage(pubkey2, vchSig, strMessage, errorMessage)) {
        LogPrintf("CmixTXQueue():Relay - Verify message failed");
        return false;
    }
...
bool CmixTXQueue::CheckSignature()
{
    ...
    if (pmn != NULL) {
        ...
        if (!miXtxSigner.VerifyMessage(pmn->pubKeyMasternode, vchSig, strMessage, errorMessage)) {
            return error("CmixTXQueue::CheckSignature() - Got bad Masternode address signature %s \n", vin.ToString().c_str());
        }
...
void CmixTXPool::RelayFinalTransaction(const int sessionID, const CTransaction& txNew)
{
    LOCK(cs_vNodes);
    BOOST_FOREACH (CNode* pnode, vNodes) {
...
void CmixTXPool::RelayIn(const std::vector<CTxDSIn>& vin, const int64_t& nAmount, const CTransaction& txCollateral, const std::vector<CTxDSOut>& vout)
{
    if (!pSubmittedToMasternode) return;
...
void CmixTXPool::RelayStatus(const int sessionID, const int newState, const int newEntriesCount, const int newAccepted, const int errorID)
{
    LOCK(cs_vNodes);
    BOOST_FOREACH (CNode* pnode, vNodes)
...
void CmixTXPool::RelayCompletedTransaction(const int sessionID, const bool error, const int errorID)
{
    LOCK(cs_vNodes);
    BOOST_FOREACH (CNode* pnode, vNodes)
...
void ThreadCheckmiXtxPool()
{
    if (fLiteMode) return; //disable all mixTX/Masternode related functionality

    // Make this thread recognisable as the wallet flushing thread
    RenameThread("loonie-mixTX");

    unsigned int c = 0;

    while (true) {
        MilliSleep(1000);
        //LogPrintf("ThreadCheckmiXtxPool::check timeout\n");
        ...
        if (masternodeSync.IsBlockchainSynced()) {
            ...
            miXtxPool.CheckTimeout();
            miXtxPool.CheckForCompleteQueue();

            if (miXtxPool.GetState() == POOL_STATUS_IDLE && c % 15 == 0) {
