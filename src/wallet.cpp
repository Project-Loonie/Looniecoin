...
// Recursively determine the rounds of a given input (How deep is the mixTX chain for a given input)
int CWallet::GetRealInputmixTXRounds(CTxIn in, int rounds) const
{
    ...
    if (wtx != NULL) {
        ...
        if (mdwi == mDenomWtxes.end()) {
            LogPrint("mixTX", "GetInputmixTXRounds INSERTING %s\n", hash.ToString());
        ...
        if (nout >= wtx->vout.size()) {
            ...
            LogPrint("mixTX", "GetInputmixTXRounds UPDATED   %s %3d %3d\n", hash.ToString(), nout, -4);
            return -4;
        }
        ...
        BOOST_FOREACH (CTxIn in2, wtx->vin) {
            if (IsMine(in2)) {
                int n = GetRealInputmixTXRounds(in2, rounds + 1);
...
int CWallet::GetInputmixTXRounds(CTxIn in) const
{
    LOCK(cs_wallet);
    int realmixTXRounds = GetRealInputmixTXRounds(in, 0);
    return realmixTXRounds > nmixTXRounds ? nmixTXRounds : realmixTXRounds;
}
...

    {
        ...
        for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it) {
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++) {
                ...
                int rounds = GetInputmixTXRounds(vin);
                fTotal += (float)rounds;
                fCount += 1;
            }
        }
...

    {
        ...
        for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it) {
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++) {
                ...
                int rounds = GetInputmixTXRounds(vin);
                nTotal += pcoin->vout[i].nValue * rounds / nmixTXRounds;
            }
        }
    }
...
bool less_then_denom(const COutput& out1, const COutput& out2)
{
    ...
    bool found1 = false;
    bool found2 = false;
    BOOST_FOREACH (int64_t d, miXtxDenominations) // loop through predefined denoms
    {
...
bool CWallet::MintableCoins()
{
    ...
    int64_t varuBalance = GetUnconfirmedBalance();
    if (mapArgs.count("-reservebalance") && !ParseMoney(mapArgs["-reservebalance"], nReserveBalance))
        return error("MintableCoins() : invalid reserve balance amount");
    if (nBalance <= (nReserveBalance+varuBalance))
        return false;
...
bool CWallet::SelectCoins(const CAmount& nTargetValue, set<pair<const CWalletTx*, unsigned int> >& setCoinsRet, CAmount& nValueRet, const CCoinControl* coinControl, AvailableCoinsType coin_type, bool useIX) const
{
    ...
    if (coinControl && coinControl->HasSelected()) {
        BOOST_FOREACH (const COutput& out, vCoins) {
            ...
            if (coin_type == ONLY_DENOMINATED) {
                ...
                if (rounds < nmixTXRounds) continue;
            }
    ...
    if (coin_type == ONLY_DENOMINATED) {
        ...
        BOOST_FOREACH (int64_t v, miXtxDenominations) {
            BOOST_FOREACH (const COutput& out, vCoins) {
...
bool CWallet::SelectCoinsByDenominations(int nDenom, int64_t nValueMin, int64_t nValueMax, std::vector<CTxIn>& vCoinsRet, std::vector<COutput>& vCoinsRet2, int64_t& nValueRet, int nmixTXRoundsMin, int nmixTXRoundsMax)
{
    vCoinsRet.clear();
    nValueRet = 0;
    ...
    BOOST_FOREACH (const COutput& out, vCoins) {
        ...
        if (nValueRet + out.tx->vout[out.i].nValue <= nValueMax) {
            ...
            int rounds = GetInputmixTXRounds(vin);
            if (rounds >= nmixTXRoundsMax) continue;
            if (rounds < nmixTXRoundsMin) continue;
...
bool CWallet::SelectCoinsDark(CAmount nValueMin, CAmount nValueMax, std::vector<CTxIn>& setCoinsRet, CAmount& nValueRet, int nmixTXRoundsMin, int nmixTXRoundsMax) const
{
    ...
    vector<COutput> vCoins;
    AvailableCoins(vCoins, true, coinControl, nmixTXRoundsMin < 0 ? ONLY_NONDENOMINATED_NOT10000IFMN : ONLY_DENOMINATED);
...
{
    return nInputAmount != 0 && nInputAmount % MIXTX_COLLATERAL == 0 && nInputAmount < MIXTX_COLLATERAL * 5 && nInputAmount > MIXTX_COLLATERAL;
}
...
bool CWallet::CreateCollateralTransaction(CMutableTransaction& txCollateral, std::string& strReason)
{
    ...
    */
    ...
    if (!SelectCoinsCollateral(vCoinsCollateral, nValueIn2)) {
        strReason = "Error: mixTX requires a collateral transaction and could not locate an acceptable input!";
        return false;
    }
    ...
    if (nValueIn2 - MIXTX_COLLATERAL - nFeeRet > 0) {
        //pay collateral charge in fees
        CTxOut vout3 = CTxOut(nValueIn2 - MIXTX_COLLATERAL, scriptChange);
    ...
    BOOST_FOREACH (CTxIn v, txCollateral.vin) {
        if (!SignSignature(*this, v.prevPubKey, txCollateral, vinNumber, int(SIGHASH_ALL | SIGHASH_ANYONECANPAY))) {
            ...
            strReason = "CmixTXPool::Sign - Unable to sign collateral transaction! \n";
            return false;
        }
        vinNumber++;
    }
            ...
            while (true) {
                ...
                if (!SelectCoins(nTotalValue, setCoins, nValueIn, coinControl, coin_type, useIX)) {
                    if (coin_type == ALL_COINS) {
                        ...
                        strFailReason = _("Unable to locate enough mixTX non-denominated funds for this transaction that are not equal 10000 LNI.");
                    } else {
                        strFailReason = _("Unable to locate enough mixTX denominated funds for this transaction.");
                        strFailReason += " " + _("mixTX uses exact denominated amounts to send funds, you might simply need to anonymize some more coins.");
                    }

                    if (useIX) {
                        strFailReason += " " + _("quickTX requires inputs with at least 6 confirmations, you might need to wait a few minutes and try again.");
                    }
...
string CWallet::PreparemixTXDenominate(int minRounds, int maxRounds)
{
    if (IsLocked())
        return _("Error: Wallet locked, unable to create transaction!");

    if (miXtxPool.GetState() != POOL_STATUS_ERROR && miXtxPool.GetState() != POOL_STATUS_SUCCESS)
        if (miXtxPool.GetEntriesCount() > 0)
            return _("Error: You already have pending entries in the mixTX pool");
    ...
    */
    if (minRounds >= 0) {
        if (!SelectCoinsByDenominations(miXtxPool.sessionDenom, 0.1 * COIN, MIXTX_POOL_MAX, vCoins, vCoins2, nValueIn, minRounds, maxRounds))
            return _("Error: Can't select current denominated inputs");
    }

    LogPrintf("PreparemixTXDenominate - preparing mixTX denominate . Got: %d \n", nValueIn);
    ...
    */
    ...
    while (nStep < nStepsMax) {
        BOOST_FOREACH (int64_t v, miXtxDenominations) {
            ...
            bool fAccepted = false;
            if ((miXtxPool.sessionDenom & (1 << 0)) && v == ((10000 * COIN) + 10000000)) {
                fAccepted = true;
            } else if ((miXtxPool.sessionDenom & (1 << 1)) && v == ((1000 * COIN) + 1000000)) {
                fAccepted = true;
            } else if ((miXtxPool.sessionDenom & (1 << 2)) && v == ((100 * COIN) + 100000)) {
                fAccepted = true;
            } else if ((miXtxPool.sessionDenom & (1 << 3)) && v == ((10 * COIN) + 10000)) {
                fAccepted = true;
            } else if ((miXtxPool.sessionDenom & (1 << 4)) && v == ((1 * COIN) + 1000)) {
                fAccepted = true;
            } else if ((miXtxPool.sessionDenom & (1 << 5)) && v == ((.1 * COIN) + 100)) {
                fAccepted = true;
            }
            if (!fAccepted) continue;
    ...
    if (miXtxPool.GetDenominations(vOut) != miXtxPool.sessionDenom) {
    ...
    miXtxPool.SendmixTXDenominate(vCoinsResult, vOut, nValueIn - nValueLeft);
...
int CMerkleTx::GetTransactionLockSignatures() const
{
    ...
    if (!IsSporkActive(SPORK_2_QUICKTX)) return -3;
    if (!fEnablequickTX) return -1;
