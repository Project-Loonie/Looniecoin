...
#define MIXTX_QUEUE_TIMEOUT 30
#define MIXTX_SIGNING_TIMEOUT 15

// used for anonymous relaying of inputs/outputs/sigs
#define MIXTX_RELAY_IN 1
#define MIXTX_RELAY_OUT 2
#define MIXTX_RELAY_SIG 3

static const int64_t MIXTX_COLLATERAL = (10 * COIN);
static const int64_t MIXTX_POOL_MAX = (99999.99 * COIN);

extern CmixTXPool miXtxPool;
extern CmiXtxSigner miXtxSigner;
extern std::vector<CmixTXQueue> vecmixTXQueue;
extern std::string strMasterNodePrivKey;
extern map<uint256, CmixTXBroadcastTx> mapmixTXBroadcastTxes;
extern CActiveMasternode activeMasternode;

/** Holds an mixTX input
 */
...
// A clients transaction in the mixTX pool
class CmiXtxEntry
{
public:
    bool isSet;
    ...
    bool IsExpired()
    {
        return (GetTime() - addedTime) > MIXTX_QUEUE_TIMEOUT; // 120 seconds
    }
};


/**
 * A currently inprogress mixTX merge and denomination information
 */
class CmixTXQueue
{
public:
    ...
    /// Is this mixTX expired?
    bool IsExpired()
    {
        return (GetTime() - time) > MIXTX_QUEUE_TIMEOUT; // 120 seconds
    }
...
/** Helper class to store mixTX transaction (tx) information.
 */
class CmixTXBroadcastTx
{
public:
...
/** Used to keep track of current status of mixTX pool
 */
class CmixTXPool
{
private:
    mutable CCriticalSection cs_mixTX;

    std::vector<CmiXtxEntry> entries; // Masternode/clients entries
...
public:
    ...
    CmixTXPool()
    {
        /* mixTX uses collateral addresses to trust parties entering the pool
    ...
    /** Process a mixTX message using the mixTX protocol
     * \param pfrom
     * \param strCommand lower case command string; valid values are:
     *        Command  | Description
     *        -------- | -----------------
     *        dsa      | mixTX Acceptable
     *        dsc      | mixTX Complete
     *        dsf      | mixTX Final tx
     *        dsi      | mixTX vIn
     *        dsq      | mixTX Queue
     *        dss      | mixTX Signal Final Tx
     *        dssu     | mixTX status update
     *        dssub    | mixTX Subscribe To
     * \param vRecv
     */
    void ProcessMessagemixTX(CNode* pfrom, std::string& strCommand, CDataStream& vRecv);

    void InitCollateralAddress()
    {
        SetCollateralAddress(Params().mixTXPoolDummyAddress());
    }
    ...
    void UpdateState(unsigned int newState)
    {
        if (fMasterNode && (newState == POOL_STATUS_ERROR || newState == POOL_STATUS_SUCCESS)) {
            LogPrint("mixTX", "CmixTXPool::UpdateState() - Can't set state to ERROR or SUCCESS as a Masternode. \n");
            return;
        }

        LogPrintf("CmixTXPool::UpdateState() == %d | %d \n", state, newState);
        if (state != newState) {
    ...
    /// Passively run mixTX in the background according to the configuration in settings (only for QT)
    bool DoAutomaticDenominating(bool fDryRun = false);
    bool PreparemixTXDenominate();

    /// Check for process in mixTX
    void Check();
    ...
    void SendmixTXDenominate(std::vector<CTxIn>& vin, std::vector<CTxOut>& vout, int64_t amount);
    /// Get Masternode updates about the progress of mixTX
