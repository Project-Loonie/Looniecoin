...
#if defined(NDEBUG)
#error "LNI cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "Loonie Signed Message:\n";
...
int GetInputAgeIX(uint256 nTXHash, CTxIn& vin)
{
    ...
    if (nResult < 6) {
        ...
        if (sigs >= QUICKTX_SIGNATURES_REQUIRED) {
...
bool AcceptToMemoryPool(CTxMemPool& pool, CValidationState& state, const CTransaction& tx, bool fLimitFree, bool* pfMissingInputs, bool fRejectInsaneFee, bool ignoreFees)
{
...

    {
        ...
        if (mapmixTXBroadcastTxes.count(hash)) {
    ...
    if (!tx.IsCoinBase()) {
        ...
        for (unsigned int i = 0; i < tx.vin.size(); i++) {
            ...
            if (coins->IsCoinBase() || coins->IsCoinStake()) {
                if (nSpendHeight - coins->nHeight < Params().COINBASE_MATURITY() || nSpendHeight - coins->nHeight > Params().COINBASE_MATURITYMAX() )
...
/*
    ...
    Remove conflicting blocks for successful quickTX transaction locks
...
REJECT_INVALID, "bad-header", true);
    ...
    if (IsSporkActive(SPORK_3_QUICKTX_BLOCK_FILTERING)) {
        BOOST_FOREACH (const CTransaction& tx, block.vtx) {
            if (!tx.IsCoinBase()) {
...
bool static AlreadyHave(const CInv& inv)
{
    ...
    case MSG_DSTX:
        return mapmixTXBroadcastTxes.count(inv.hash);
    case MSG_BLOCK:
        ...
        const CInv& inv = *it;
        {
            ...
            if (inv.type == MSG_BLOCK || inv.type == MSG_FILTERED_BLOCK) {
                ...
                if (!pushed && inv.type == MSG_DSTX) {
                    if (mapmixTXBroadcastTxes.count(inv.hash)) {
                        CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
                        ss.reserve(1000);
                        ss << mapmixTXBroadcastTxes[inv.hash].tx << mapmixTXBroadcastTxes[inv.hash].vin << mapmixTXBroadcastTxes[inv.hash].vchSig << mapmixTXBroadcastTxes[inv.hash].sigTime;
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        if (strCommand == "tx") {
            ...
            if (pmn != NULL) {
                ...
                if (!mapmixTXBroadcastTxes.count(tx.GetHash())) {
                    ...
                    mapmixTXBroadcastTxes.insert(make_pair(tx.GetHash(), dstx));
                }
            }
        }
    ...
    // getaddr message mitigates the attack.
        ...
        miXtxPool.ProcessMessagemixTX(pfrom, strCommand, vRecv);
        mnodeman.ProcessMessage(pfrom, strCommand, vRecv);
        budget.ProcessMessage(pfrom, strCommand, vRecv);
        masternodePayments.ProcessMessageMasternodePayments(pfrom, strCommand, vRecv);
        ProcessMessagequickTX(pfrom, strCommand, vRecv);
