...
void WalletModel::pollBalanceChanged()
{
    ...
    if (fForceCheckBalanceChanged || chainActive.Height() != cachedNumBlocks || nmixTXRounds != cachedmixTXRounds || cachedTxLocks != nCompleteTXLocks) {
        fForceCheckBalanceChanged = false;

        // Balance and number of transactions might have changed
        cachedNumBlocks = chainActive.Height();
        cachedmixTXRounds = nmixTXRounds;
...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
...

    {
        ...
        if (recipients[0].usequickTX && total > GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("quickTX doesn't support sending values that high yet. Transactions are currently limited to %1 LNI.").arg(GetSporkValue(SPORK_5_MAX_VALUE)),
                CClientUIInterface::MSG_ERROR);
            return TransactionCreationFailed;
        }

        bool fCreated = wallet->CreateTransaction(vecSend, *newTx, *keyChange, nFeeRequired, strFailReason, coinControl, recipients[0].inputType, recipients[0].usequickTX);
        transaction.setTransactionFee(nFeeRequired);

        if (recipients[0].usequickTX && newTx->GetValueOut() > GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("quickTX doesn't support sending values that high yet. Transactions are currently limited to %1 LNI.").arg(GetSporkValue(SPORK_5_MAX_VALUE)),
                CClientUIInterface::MSG_ERROR);
            return TransactionCreationFailed;
        }
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction& transaction)
{
...

    {
        ...
        if (!wallet->CommitTransaction(*newTx, *keyChange, (recipients[0].usequickTX) ? "ix" : "tx"))
