...
Value mixTX(const Array& params, bool fHelp)
{
    if (fHelp || params.size() == 0)
        throw runtime_error(
            "mixTX <loonieaddress> <amount>\n"
            "loonieaddress, reset, or auto (AutoDenominate)"
    ...
    if (params[0].get_str() == "auto") {
        if (fMasterNode)
            return "miXtx is not supported from masternodes";

        return "DoAutomaticDenominating " + (miXtxPool.DoAutomaticDenominating() ? "successful" : ("failed: " + miXtxPool.GetStatus()));
    }

    if (params[0].get_str() == "reset") {
        miXtxPool.Reset();
        return "successfully reset mixTX";
    }

    if (params.size() != 2)
        throw runtime_error(
            "mixTX <loonieaddress> <amount>\n"
            "loonieaddress, denominate, or auto (AutoDenominate)"
            "<amount> is a real and will be rounded to the next 0.1" +
            HelpRequiringPassphrase());
...
Value getpoolinfo(const Array& params, bool fHelp)
{
    ...
    obj.push_back(Pair("state", miXtxPool.GetState()));
    obj.push_back(Pair("entries", miXtxPool.GetEntriesCount()));
    obj.push_back(Pair("entries_accepted", miXtxPool.GetCountEntriesAccepted()));
    return obj;
}
