...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-port=<port>", strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), 33447, 33547));
    strUsage += HelpMessageOpt("-proxy=<ip:port>", _("Connect through SOCKS5 proxy"));
    strUsage += HelpMessageOpt("-seednode=<ip>", _("Connect to a node to retrieve peer addresses, and disconnect"));
	strUsage += HelpMessageOpt("--changeaddress=<LNI Address>", _("Add your LNI changeaddress"));
    ...
    strUsage += HelpMessageOpt("-zmqpubhashtxlock=<address>", _("Enable publish hash transaction (locked via quickTX) in <address>"));
    strUsage += HelpMessageOpt("-zmqpubrawblock=<address>", _("Enable publish raw block in <address>"));
    strUsage += HelpMessageOpt("-zmqpubrawtx=<address>", _("Enable publish raw transaction in <address>"));
    strUsage += HelpMessageOpt("-zmqpubrawtxlock=<address>", _("Enable publish raw transaction (locked via quickTX) in <address>"));
#endif
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, mempool, net, loonie, (mixTX, quicktx, masternode, mnpayments, mnbudget)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all LNI specific functionality (Masternodes, mixTX, quickTX, Budgeting) (0-1, default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-autoposdustmerge=<n>", strprintf(_("Combine dust transactions into one stake transaction (0-1, default: %u)"), 1));
    if (GetBoolArg("-help-debug", false)) {
    ...
    strUsage += HelpMessageOpt("-masternodeaddr=<n>", strprintf(_("Set external address:port to get to this masternode (example: %s)"), "128.127.106.235:33447"));
    strUsage += HelpMessageOpt("-budgetvotemode=<mode>", _("Change automatic finalized budget voting behavior. mode=auto: Vote for only exact finalized budget match to my generated budget. (string, default: auto)"));

    strUsage += HelpMessageGroup(_("mixTX options:"));
    strUsage += HelpMessageOpt("-enablemixTX=<n>", strprintf(_("Enable use of automated mixTX for funds stored in this wallet (0-1, default: %u)"), 0));
    strUsage += HelpMessageOpt("-mixTXrounds=<n>", strprintf(_("Use N separate masternodes to anonymize funds  (2-8, default: %u)"), 2));
    strUsage += HelpMessageOpt("-anonymizeloonieamount=<n>", strprintf(_("Keep N LNI anonymized (default: %u)"), 0));
    strUsage += HelpMessageOpt("-liquidityprovider=<n>", strprintf(_("Provide liquidity to mixTX by infrequently mixing coins on a continual basis (0-100, default: %u, 1=very frequent, high fees, 100=very infrequent, low fees)"), 0));
	
	strUsage += HelpMessageGroup(_("quickTX options:"));
    strUsage += HelpMessageOpt("-enablequicktx=<n>", strprintf(_("Enable quicktx, show confirmations for locked transactions (bool, default: %s)"), "true"));
    strUsage += HelpMessageOpt("-quicktxdepth=<n>", strprintf(_("Show N confirmations for a successfully locked transaction (0-9999, default: %u)"), nquickTXDepth));
    ...
    strUsage += HelpMessageOpt("-rpcport=<port>", strprintf(_("Listen for JSON-RPC connections on <port> (default: %u or testnet: %u)"), 33448, 33548));
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2015-%i The Loonie Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
/** Sanity checks
 *  Ensure that LNI is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    if (!GetBoolArg("-enablequicktx", fEnablequickTX)) {
        if (SoftSetArg("-quicktxdepth", 0))
            LogPrintf("AppInit2 : parameter interaction: -enablequicktx=false -> setting -nquickTXDepth=0\n");
    }
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Loonie Core is shutting down."));
...
#endif
    // Make sure only a single LNI process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Loonie Core is probably already running."), strDataDir));
...
// ********************************************************* Step 8: load wallet
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK) {
            if (nLoadWalletRet == DB_CORRUPT)
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Loonie Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE) {
                strErrors << _("Wallet needed to be rewritten: restart Loonie Core to complete") << "\n";
    ...
    if (fMasterNode) {
        LogPrintf("IS MIXTX MASTER NODE\n");
        ...
        if (!strMasterNodePrivKey.empty()) {
            ...
            if (!miXtxSigner.SetKey(strMasterNodePrivKey, errorMessage, key, pubkey)) {
    ...
    fEnablemixTX = GetBoolArg("-enablemixTX", false);

    nmixTXRounds = GetArg("-mixTXrounds", 2);
    if (nmixTXRounds > 16) nmixTXRounds = 16;
    if (nmixTXRounds < 1) nmixTXRounds = 1;

    nLiquidityProvider = GetArg("-liquidityprovider", 0); //0-100
    if (nLiquidityProvider != 0) {
        miXtxPool.SetMinBlockSpacing(std::min(nLiquidityProvider, 100) * 15);
        fEnablemixTX = true;
        nmixTXRounds = 99999;
    }
    ...
    fEnablequickTX = GetBoolArg("-enablequicktx", fEnablequickTX);
    nquickTXDepth = GetArg("-quicktxdepth", nquickTXDepth);
    nquickTXDepth = std::min(std::max(nquickTXDepth, 0), 60);

    //lite mode disables all Masternode and mixTX related functionality
    ...
    LogPrintf("nquickTXDepth %d\n", nquickTXDepth);
    LogPrintf("mixTX rounds %d\n", nmixTXRounds);
    LogPrintf("Anonymize LNI Amount %d\n", nAnonymizeLoonieAmount);
    LogPrintf("Budget Mode %s\n", strBudgetMode.c_str());

    /* Denominations

       A note about convertability. Within mixTX pools, each denomination
    ...
    */
    miXtxDenominations.push_back((10000 * COIN) + 10000000);
    miXtxDenominations.push_back((1000 * COIN) + 1000000);
    miXtxDenominations.push_back((100 * COIN) + 100000);
    miXtxDenominations.push_back((10 * COIN) + 10000);
    miXtxDenominations.push_back((1 * COIN) + 1000);
    miXtxDenominations.push_back((.1 * COIN) + 100);
    /* Disabled till we need them
    miXtxDenominations.push_back( (.01      * COIN)+10 );
    miXtxDenominations.push_back( (.001     * COIN)+1 );
    */

    miXtxPool.InitCollateralAddress();

    threadGroup.create_thread(boost::bind(&ThreadCheckmiXtxPool));
